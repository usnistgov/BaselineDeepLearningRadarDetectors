...
from lib.testProcess import constructDatasetFilesVars, createMaxHoldSpectro, loadModel, generateTestResults
#%%
modelDir='../trainedModels'
modelsToTest={
    "CNN3-SpectroMaxHold": True,
    "ResNet50-SpectroMaxHold": True,
    "Xception-SpectroMaxHold": True,
    "MobileNetV2-SpectroMaxHold": True
}
...
for modelNamePath in modelNamePaths:
    ...
    for fileIndex in range(len(fileNames)):
        ...
        for sigIndex in range(subsetSignals.shape[0]):#range(subsetSignals.shape[0]):#np.nditer(highSNRIndex):#range(subsetSignals.shape[0]):
            spectroN=createMaxHoldSpectro(subsetSignals[sigIndex])
